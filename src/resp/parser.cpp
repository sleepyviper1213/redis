...
Parser::parse_recursive(std::string_view data, size_t &pos) {
	...
	if (line_end < 1 || data[line_end - 1] != '\r') FAILED(MISSING_CRLF);
	...
	case '$': {
		...
		if (len < -1 || len >= RESP_MAX_SIZE) FAILED(INVALID_BULK_LENGTH);
	...
	case '*': {
		const auto len = TRY_FROM(parse_integer(payload), RespError);
		if (len == -1) return Value::from_null();
		if (len < -1 || len >= RESP_MAX_SIZE) FAILED(INVALID_ARRAY_LENGTH);
