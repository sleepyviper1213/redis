...
// Specialization when Variant is std::variant<...>
template <typename T, typename... Ts>
struct has_alternative<T, std::variant<Ts...>>
	: std::disjunction<std::is_same<T, Ts>...> {};

template <typename T, typename Variant>
concept variant_contains = has_alternative<T, Variant>::value;
...
public:
	...
	/// - Array: Array
	/// - double: Double
	using Variant = std::variant<std::string, int64_t, Array, double>;
	 ...
	 */
	template <typename T>
		requires variant_contains<T, Variant>
	Value(Type type, T t) : type_(type), data_(std::move(t)) {}

	template <std::ranges::input_range Range>
	constexpr Value(Range &&r)
		: type_(Type::ARRAY),
		  data_{Array(std::from_range, std::forward<Range>(r))} {}
	...
	template <std::ranges::input_range Range>
	ErrorRefOr<Value> insert_range(Range &&r) {
		auto *array = std::get_if<Array>(&data_);
		if (!array) BAIL(RedisError::Kind::WRONG_TYPE);

		array->append_range(std::forward<Range>(r));
		return *this;
	}
